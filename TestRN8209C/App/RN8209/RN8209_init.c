...
static void RN8209_Set_SysCtrl(EN_RN8209_SF SpecialFunc)
{
...
static u16 RN8209_Set_IB_Gain(void)
{
	...
	RN8209_Reg.Ctrl.IBGain = 0x8000 - 1; 		//B通道增益翻倍

	RN8209_Write_Reg(ADDR_IBGain);

	//计算校验
	CheckSum += RN8209_Reg.Ctrl.IBGain;
...
static u16 RN8209_Set_Ctrl_Regs(void)
{
	...
	RN8209_Reg.Ctrl.EMUCON2.Value = 0;
	...
	//计算校验
	CheckSum += RN8209_Reg.Ctrl.SYSCON.Value;
	CheckSum += RN8209_Reg.Ctrl.EMUCON.Value;
	CheckSum += RN8209_Reg.Ctrl.EMUCON2.Value;
	CheckSum += RN8209_Reg.Ctrl.HFConst;
...
static u16 RN8209_Set_Power_Start(void)
{
	...
	RN8209_Reg.Ctrl.PStart = 0x60;
	RN8209_Reg.Ctrl.DStart = 0x60;//0x120;

	RN8209_Write_Reg(ADDR_PStart);
	RN8209_Write_Reg(ADDR_DStart);

	//计算校验
	CheckSum += RN8209_Reg.Ctrl.PStart;
	CheckSum += RN8209_Reg.Ctrl.DStart;
...
static u16 RN8209_Set_DC_Offset(void)
{
	...
	RN8209_Reg.Ctrl.DCIAH = Storage_RN8209.CtrlReg.DCIAH;
	RN8209_Reg.Ctrl.DCIBH = Storage_RN8209.CtrlReg.DCIBH;
	RN8209_Reg.Ctrl.DCUH = Storage_RN8209.CtrlReg.DCUH;
	RN8209_Reg.Ctrl.DCL = Storage_RN8209.CtrlReg.DCL;
	...
	//计算校验
	CheckSum += RN8209_Reg.Ctrl.DCIAH;
	CheckSum += RN8209_Reg.Ctrl.DCIBH;
	CheckSum += RN8209_Reg.Ctrl.DCUH;
	CheckSum += RN8209_Reg.Ctrl.DCL;
...
static u16 RN8209_Set_EVD(void)
{
	...
	RN8209_Reg.Ctrl.IARMSOS = Storage_RN8209.CtrlReg.IARMSOS;
	RN8209_Reg.Ctrl.IBRMSOS = Storage_RN8209.CtrlReg.IBRMSOS;

	RN8209_Write_Reg(ADDR_IARMSOS);		//电流a有效值偏差
	RN8209_Write_Reg(ADDR_IBRMSOS);		//电流b有效值偏差

	//计算校验
	CheckSum += RN8209_Reg.Ctrl.IARMSOS;
	CheckSum += RN8209_Reg.Ctrl.IBRMSOS;
...
static u16 RN8209_Set_Gain(void)
{
	...
	RN8209_Reg.Ctrl.GPQA = Storage_RN8209.CtrlReg.GPQA;
	RN8209_Reg.Ctrl.GPQB = Storage_RN8209.CtrlReg.GPQB;

	RN8209_Write_Reg(ADDR_GPQA);
	RN8209_Write_Reg(ADDR_GPQB);

	//计算校验
	CheckSum += RN8209_Reg.Ctrl.GPQA;
	CheckSum += RN8209_Reg.Ctrl.GPQB;
...
static u16 RN8209_Set_Power_Offset(void)
{
	...
	RN8209_Reg.Ctrl.APOSA = Storage_RN8209.CtrlReg.APOSA;
	RN8209_Reg.Ctrl.APOSB = Storage_RN8209.CtrlReg.APOSB;

	RN8209_Write_Reg(ADDR_APOSA);
	RN8209_Write_Reg(ADDR_APOSB);

	//计算校验
	CheckSum += RN8209_Reg.Ctrl.APOSA;
	CheckSum += RN8209_Reg.Ctrl.APOSB;
...
static u16 RN8209_Init_Func(EN_RN8209_INIT_FUNC Func)
{
	...
	RN8209_Set_SysCtrl(SF_UNLOCK);

	switch(Func)
	{
		case RN8209_RESET:
			RN8209_Set_SysCtrl(SF_RESET);
			break;
		case RN8209_SET_CTRL_REG:
			CheckSum = RN8209_Set_Ctrl_Regs();
			break;
...
void RN8209_Init(EN_RN8209_INIT_TYPE Type)
{
	...
	switch(Type)
	{
		...
		case RN8209_INIT_CALIBRATE_DC_OFFSET:
			CheckSum += RN8209_Init_Func(RN8209_SET_CTRL_REG);
